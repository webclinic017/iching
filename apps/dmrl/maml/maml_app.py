...
class MamlApp(object):
    ...
    def get_meta_batch(self, meta_batch_size, k_shot, q_query, data_loader, iterator):
        ...
        for _ in range(meta_batch_size):
            ...
            train_data = task_data[:, :k_shot].reshape(-1, 50)
    ...
    def train_batch_org(self, model, optimizer, x, y, n_way, k_shot, 
                q_query, loss_fn, inner_train_steps= 1, 
                inner_lr = 0.4, train = True):
        """
        Args:
    ...
    def get_meta_batch_org(self, meta_batch_size, k_shot, q_query, data_loader, iterator):
        data = []
        y = torch.tensor([])
        for _ in range(meta_batch_size):
            try:
                ...
                iterator = iter(data_loader)
                task_data, y = iterator.next()
            print('task_data: {0}={1};'.format(type(task_data), task_data.shape))
            print('y: {0}={1};'.format(type(y), y.shape))
