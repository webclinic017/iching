...
class IqttHumanRender(object):
    ...
    def _render_trades(self, trades):
        for item in trades['trade_history']:
            ...
            if item['trade_mode'] == AppConfig.TRADE_MODE_BUY:
                color = IqttHumanRender.BUY_TEXT_COLOR
                msg = 'BUY:{0}*{1:.2f}'.format(item['quant'], item['price'])
            else:
                color = IqttHumanRender.SELL_TEXT_COLOR
    ...
    def _render_price(self, trades):
        ...
        for idx in range(upper_limit):
            ...
            self.price_ax = self.draw_candlestick(self.price_ax, idx, data, IqttHumanRender.OHLC_UP, IqttHumanRender.OHLC_DOWN)  
        ...
        self.price_ax.set_ylim(ylim[0] - (ylim[1] - ylim[0]) * IqttHumanRender.VOLUME_CHART_HEIGHT, ylim[1])
    ...
    def _render_volume(self, trades): #current_step, net_worth, dates, 
        ...
        self.volume_ax.bar(np.array(up_idxs), np.array(up_volumes), color=IqttHumanRender.OHLC_UP,
            alpha=0.4, width=1, align='center')
        self.volume_ax.bar(np.array(down_idxs), np.array(down_volumes), color=IqttHumanRender.OHLC_DOWN,
            alpha=0.4, width=1, align='center')
        # Cap volume axis height below price chart and hide ticks
        self.volume_ax.set_ylim(0, max(volume) / IqttHumanRender.VOLUME_CHART_HEIGHT)
